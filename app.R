...
ors <- ors_api(options = list(api_key = ors_api_key))
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    textInput("clicked_lat", "Clicked Latitude", value = ""),
    textInput("clicked_lng", "Clicked Longitude", value = ""),
    tags$style(HTML("#clicked_lat, #clicked_lng {display:none;}")),  # Hiding the text inputs
    width = 250
  ),
  dashboardBody(
...
get_directions <- function(start_lat, start_lon, end_lat, end_lon) {
  coordinates <- list(
    c(start_lon, start_lat),
    c(end_lon, end_lat)
  )
  
  route <- ors_directions(ors, coordinates = coordinates, profile = "driving-car", format = "geojson")
  return(route)
}
...
server <- function(input, output, session) {
  ...
  # Observe the geolocation of user and pools
  
  observe({
    req(input$user_lat, input$user_lon, input$title != "Alle")
    selected_pool <- df_pools[df_pools$Title == input$title, ]
    if (nrow(selected_pool) > 0) {
      route <- get_directions(input$user_lat, input$user_lon, selected_pool$Latitude, selected_pool$Longitude)
      
      # Update the map with the new route
      leafletProxy("map") %>%
        clearLayers() %>%
        addTiles() %>%
        addGeoJSON(route, group = "route") %>%
        addCircleMarkers(data = selected_pool, ~Longitude, ~Latitude, popup = ~paste(Title, "<br>", Wassertemperatur, "°C"), group = "pools") %>%
        addMarkers(lng = input$user_lon, lat = input$user_lat, popup = "Your location", group = "location") %>%
        fitBounds(lng1 = input$user_lon, lat1 = input$user_lat, 
                  lng2 = selected_pool$Longitude, lat2 = selected_pool$Latitude)
    }
  })
  ...
  # Capture the click event on pool markers
  observeEvent(input$map_marker_click, {
    clicked_pool <- input$map_marker_click
    clicked_lat <- clicked_pool$lat
    clicked_lng <- clicked_pool$lng
    
    # Store the clicked coordinates
    updateTextInput(session, "clicked_lat", value = clicked_lat)
    updateTextInput(session, "clicked_lng", value = clicked_lng)
  })
  
  # Calculate route using OpenRouteService API
  observeEvent(input$clicked_lat, {
    req(input$clicked_lat, input$clicked_lng, input$user_lat, input$user_lon)
    
    user_location <- c(input$user_lon, input$user_lat)
    pool_location <- c(as.numeric(input$clicked_lng), as.numeric(input$clicked_lat))
    
    route <- ors_directions(
      coordinates = list(user_location, pool_location),
      profile = "foot-walking",
      api_key = "ors_api_key" # Replace with your OpenRouteService API key
    )
    
    # Extract route coordinates
    route_coords <- route$routes[[1]]$geometry$coordinates
    route_df <- data.frame(
      lng = sapply(route_coords, `[[`, 1),
      lat = sapply(route_coords, `[[`, 2)
    )
    
    # Add the route to the map
    leafletProxy("map") %>%
      clearGroup("route") %>%
      addPolylines(data = route_df, lng = ~lng, lat = ~lat, color = "red", group = "route")
  })
}
